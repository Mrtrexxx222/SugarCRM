...
var window = this, //acelera las referencias a window
	...
	//_jsframe = window.jsframe,
	query = !!document.querySelectorAll; //comprueba si querySelectorAll esta disponible para su uso

jsframe = _jsframe = window.jsframe = window.$ = function(sel, context){
	return new jsframe(sel, context);
};

/**
 * Constructor jsframe; permite varios tipos de selector
 * 
 ...
 */
jsframe = function(sel, context){
	//si no se pasa ningun selector se usa document
	sel = sel || document;

	this.version = 'jsframe v1.5b',
	...
	if(typeof(sel) === 'string'){
		...
		if(match && match[1]){
			...
			if(match[3]){ //id, con o sin # ej. #myid o myid
				...
				this.nodes = jsframe.funcs.getElementsByClassName(match[4], this.context);
				...
				if(query && this.context === document){
					this.nodes = jsframe.funcs.selector(sel);
				}else{
					//pasarselo a Sizzle (mas eficiente con navegadores antiguos)
					try{this.nodes = jsframe.find(sel,this.context);}
	...
	}else if(jsframe.funcs.isArray(sel)){
		this.nodes = sel;
	}else{
		this.nodes = jsframe.funcs.makeArray(sel);
	}
...
jsframe.prototype = {
	...
	navegador: (navigator.userAgent.toLowerCase().match( /.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/ ) || [0,'0'])[1], //version
		...
		if(attribute){
			this.each(function(){
				...
				try{
					...
					jsframe.funcs.debugShow('-jsframe::hasAtts- error: "' + err + '"');
					//return false;
				}
			});
		}
...
//funciones estaticas referenciables por si mismas,  ej. var a = jsframe.funcs.getDetails(obj)
//tambien extiende jsframe.prototype para que cualquier instancia tenga estos metodos
jsframe.funcs = {
	/**
	...
	listoFuncs: [],
	runListo: function (){for (var i = 0; i < this.listoFuncs.length; i++) this.listoFuncs[i]();},
	...
	ready: function (f){
		...
		if(this.listoFuncs.push(f) > 1) return;
		if(this.msie){
			(function (){
				try {document.documentElement.doScroll('left'); jsframe.funcs.runListo();}
				catch (err){setTimeout(arguments.callee, 0);}
			})();
		}
		else if(this.webkit){
			var t = setInterval(function (){
					if (/^(loaded|complete)$/.test(document.readyState)) clearInterval(t), jsframe.funcs.runListo();
				}, 0);
			}
	},
	 ...
	 * 
	 * @since 2011-03-24
	 * @param string text Texto informativo (de debug) a mostrar en el elemento creado
	 * @return object
	 */
	debugShow: function(text){
		if(!window.debug) return(false);
		if(!text) var text = 'jsframe debug';

		//contenedor de los mensajes de debug
		var e = document.getElementById('jsframeDebug'); //TODO en firefox adquiere los metodos de jsframe, en ie no
		if(!e){
			e = document.createElement('ul');
			e.className = 'jsframeDebug';
			e.id = 'jsframeDebug'; //TODO diferenciar id's, por si se crea mas de uno
			_jsframe('<body>').insertAfter(e);
		}

		//cada uno de los mensajes de debug
		var m = document.createElement('li');
		m.className = 'jsframeDebug';
		m.appendChild(document.createTextNode(text));
		_jsframe('<body>').append(m, e);
	...
	extendTrads: function(obj){
		for(var lang in obj){
			if(!jsframe.funcs._trads[lang]) jsframe.funcs._trads[lang] = {};

			for(var key in obj[lang]){
				jsframe.funcs._trads[lang][key] = obj[lang][key];
			}
		}
		return;
	},
	...
	_tradR: function(func, langDef){
		if(!jsframe.funcs.isArray(func)) func = [func];
...
navigator.systemLanguage : OS language*/
		...
		if(jsframe.funcs._trads[lang] && jsframe.funcs._trads[lang][func[0]]) func[0] = jsframe.funcs._trads[lang][func[0]];
		else if(jsframe.funcs._trads[langDef] && jsframe.funcs._trads[langDef][func[0]]) func[0] = jsframe.funcs._trads[langDef][func[0]]; //lenguaje por defecto

		return(jsframe.funcs.sprintf.apply(this, func));
	},
	 ...
	 *
	 * @todo Por ahora solo se reconoce '%s' (string) y '%%' (%)
	 * @since 2011-06-20
	 * @return string
	 */
	sprintf: function(){
		...
		while(sust = regExp.exec(cadena)){
			switch(sust[1]){
				...
				case '%s': //cadena de texto
					cadena = cadena.substr(0, sust.index) + arguments[++cont] + cadena.substr(sust.index + 2);
					break;
				default:
	 ...
	 *
	 * @since 2011-05-10
	 * @param string scrPath Ruta absoluta ("http://ruta/script.js") o relativa a donde se encuentre "jsframe_core.js" (como "ruta/script.js")
	 */
	loadScript: function(scrPath){
		var scrId = 'jsframe_script_' + scrPath.replace(/[^a-zA-Z\d_]+/, '');
		if(document.getElementById(scrId)){
			jsframe.funcs.debugShow('-jsframe::loadScript- Script (id->' + scrId + ') ya cargado.');
			return(false); //ya cargado
		}

		//si se ha pasado una ruta no absoluta se le suma la misma ruta en que se encuentre "jsframe_core.js"
		if(scrPath.indexOf('http://') === -1){
			var temp_js = new RegExp("(^|(.*?\\/))(jsframe_core\.js)(\\?|$)");
			...
			for(var i = 0, lon = scripts.length; i < lon ; i++){
				...
				if(src){
					...
					if(srcMatch){
						scrPath = srcMatch[1] + scrPath; //pone la misma ruta que "jsframe_core.js"
		...
		/*try{
			...
			jsframe.funcs.debugShow('-jsframe::loadScript- Script (id->' + scrId + ') leido con "document.write".');
		}
		catch(e){*/
			...
			if(script.readyState){ //IE
				script.onreadystatechange = function(){
					if(script.readyState == "loaded" || script.readyState == "complete"){
						...
						var scriptQueue = jsframe.funcs.loadScriptQueue;
						jsframe.funcs.loadScriptQueue = [];
						...
						for(mt in scriptQueue){
							try{
								...
								jsframe.funcs.debugShow('-jsframe::loadScript- No se ha podido ejecutar un método. ' + e);
								return;
							}
						}
						jsframe.funcs.loadScriptQueue = [];
	...
	// for older browser support use sizzle.js and map the public methods to jsframe methods e.g
	// at the bottom of the sizzle file replace the jQuery methods with the following
	/*
		// EXPOSE Sizzle to jsframe.js

		window.Sizzle = Sizzle;

		jsframe.find = Sizzle;
		jsframe.filter = Sizzle.filter;
		jsframe.expr = Sizzle.selectors;
		jsframe.expr[":"] = jsframe.expr.filters;
	*/
	...
	//TODO revisar uso de this
	*/
	getStyle: function(cssRule, elem){
...
//extiende el prototipo con los metodos encontrados en jsframe.funcs
jsframe.funcs.extend(jsframe.prototype,jsframe.funcs);
...
jsframe.funcs.extend(jsframe.prototype, {
	noConflict: function(deep){
		if(window.$ === jsframe) window.$ = _$;
		if(deep && window.jsframe === jsframe) window.jsframe = _jsframe;
		return jsframe;
	}
});

window.nombreEvento = ''; //guarda el nombre del ultimo evento disparado mediante gestion de eventos (a continuacion)

/**
 * Esta funcion es llamada en cada evento; independiente de objetos jsframe.
 * Tiene una gemela jsframe.triggerEvento() (deshabilitada)
 * 'this' es el objeto que lo haya llamado
 * 
 * @param ev Nombre del evento
 * @return void
 */
window.triggerEvento = function (ev){
	//return(true);
};
...
/**
 * Aumentando las capacidades de jsframe
 * 
 * Gestion de eventos
 */
jsframe.funcs.extend(jsframe.prototype, {
	 ...
	 * Tiene una gemela window.triggerEvento()
	 * 
	 * @param e Evento
	 * @return boolean
	 */
	/*triggerEvento: (function (e){
		return(true);
	}),*/
...
The main difference from the traditional model is that multiple event handlers can be registered for the same event. The useCapture option can also be used to specify that the handler should be called in the capture phase instead of the bubbling phase. This model is supported by Mozilla, Opera, Safari, Chrome and Konqueror.
	...
	addEvent: function (evento, funcion, capt){
		//TODO concatena a la definicion de la funcion la asignacion de "nombreEvento" para controlar que evento ha sido disparado
		/*funcion = eval(funcion.toString() + " window.nombreEvento='" + evento + "';");
		...
		if(document.addEventListener){ //w3c
			this.each(
				function (evt, func, ct){
					...
					switch(evt){
						case 'mouseenter':
							this.addEventListener('mouseover', jsframe.funcs.mouseEnter(func), ct);
							break;
						case 'mouseleave':
							this.addEventListener('mouseout', jsframe.funcs.mouseEnter(func), ct);
							break;
						case 'mousewheel':
							//if we're dealing with a Gecko browser, the event name and handler need some adjustment
							var ua = navigator.userAgent.toLowerCase(); //TODO identificar con el propio jsframe
			...
			if(window.triggerEvento) this.each(
				function (evt, ct){
					if(!this || this.nodeType == 3 || this.nodeType == 8) return undefined; //sin eventos en nodos texto y comentarios
					this.addEventListener(evt, function (){window.triggerEvento.call(this, evt);}, ct);
				}, [evento, capt]);
			...
			if(window.triggerEvento) this.each(
				function (evt){
					var elemento = this, clave = elemento + evt + window.triggerEvento;

					if(!elemento || elemento.nodeType == 3 || elemento.nodeType == 8) return undefined; //sin eventos en nodos texto y comentarios

					elemento['e' + clave] = function (){window.triggerEvento.call(elemento, evt);};
					elemento[clave] = function(){elemento['e' + clave](window.event);};
					elemento.attachEvent('on' + evt, elemento[clave]);
				}, [evento]);
		}
			...
			if(window.triggerEvento) this.each(
				function (evt){
					if(!this || this.nodeType == 3 || this.nodeType == 8) return undefined; //sin eventos en nodos texto y comentarios
					this['on' + evt] = function (){window.triggerEvento.call(this, evt);};
				}, [evento]);
		}
...
jsframe.funcs.extend(jsframe.prototype, {
	 ...
	 */
	clone: function (obj){
		var obj = obj || this; //se usa el objeto jsframe actual si no se pasa ninguno
	 ...
	 * @since 2010-12-14
	 * @param nodo Elemento a insertar o matriz con sus caracteristicas (en este orden: tag (sin llaves angulares), [texto|NULL], [clase css|NULL], [id|NULL])
	 * @param ancla Elemento al que se añadira nodo; si va vacio se usa this (los nodos de jsframe)
	 * @return object
	 */
	append: function(nodo, ancla){
		nodo = nodo || this; //se usa el objeto jsframe actual si no se pasa ninguno (o si se pasa null); util para clonar, por ej.
	...
	prepend: function(nodo, ancla){
		nodo = nodo || this; //se usa el objeto jsframe actual si no se pasa ninguno; util para clonar, por ej.
		...
		return this.prevObject || jsframe( [] );
	}
...
jsframe.funcs.extend(jsframe.prototype, {
	 ...
	 * @return object
	 */
	//TODO devolver los id para controlarlos fuera; this en la funcion pasada deben ser los nodos jsframe?
...
jsframe.funcs.extend(jsframe.funcs, {
	...
	hrefHashBang: function(){
		//_jsframe().ready(function (){
			_jsframe('<a>').addEvent('click', function(e){
				alert(this.href);
				if(this.href[0] == "/"){
					jsframe(this).noPropagarEvento(e);
...
jsframe.funcs.extend(jsframe.prototype, {
	...
	loadMethod: function(method, args, library){
		if(!library){
			switch(method){
				case 'ajax':
					library = 'jsframe_ajax.js';
					break;
				case 'formulario':
					library = 'jsframe_formulario.js';
					break;
				case 'move':
					library = 'jsframe_move.js';
					break;
				case 'ratingStars':
					library = 'jsframe_rating.js';
					break;
				case 'rtb':
					library = 'jsframe_rtb.js';
					break;
				default:
					library = false;
					jsframe.funcs.debugShow('-jsframe::loadMethod- Intenta cargar dinamicamente una librería desconocida para el metodo: ' + method);
			}
		}
		...
		if(library){
			jsframe.funcs.loadScriptQueue.push({'fn':tempCall,'ctx':this});
			jsframe.funcs.loadScript('packer.php?scriptJs=' + library); //version con empaquetador/minificador "class.JavaScriptPacker.php"
			//if(library) jsframe.funcs.loadScript(library, tempCall, this); //version sin empaquetador
		}
	},
	...
	/*ajax: function(){
		...
		ret = jsframe.funcs.loadScript('jsframe_ajax.js', tempCall, this);
...
//extiende con los metodos encontrados en jsframe.funcs, permite llamadas mas directas: jsframe.metodo();
jsframe.funcs.extend(jsframe, jsframe.funcs);
