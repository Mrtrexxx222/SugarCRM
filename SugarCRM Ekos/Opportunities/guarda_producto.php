...
if($seccion == 'D')
{
	...
	{
		$validador = valida_asignados($oportunidad,$id_usuario,$cuenta,$guarda_o_no,$acumulador,$op_duplicado,$duenio);
...
//FUNCION QUE GRABA LOS PRODUCTOS, RETORNARA EL ID DE LOS PRODUCTO QUE DEBEN SER VALIDADO EN EL ARCHIVO: valida_asig_presup.php
function save_productos($id_op,$id_producto,$fecha_cierre,$descripcion,$guarda_o_no,$forma_pago,$estatus,$duenio,$valor_prosp,$id_cta,$cuenta)
{
	//instancia de la base
	$db =  DBManagerFactory::getInstance();
	//VARIABLES
	...
	$SQL_1 = "SELECT name FROM opportunities WHERE id = '".$id_op."'";
	$resultSQL_1 = $db->query($SQL_1, true, 'Error selecting the opportunity record');
	...
	$query = "SELECT b.name as categoria, a.name as producto FROM product_templates a, product_categories b 
			  where a.category_id = b.id and a.id = '".$id_producto."' and a.deleted = 0";
	$result = $db->query($query, true, 'Error selecting the category record');
		if($row=$db->fetchByAssoc($result))
		{
			valida_auditoria_dt_op($id_op,$id_producto,$row0['id'],$estatus,$valor,$oportunidad,$mes,$anio,$id_usuario,$usuario,$id_cta,$cuenta, $forma_pago, $descripcion);
			$categoria = $row['categoria'];
			$query2 = "SELECT name, probability FROM opportunities where id = '".$id_op."'"; 
			...
			if($row2=$db->fetchByAssoc($result2))//SI LA OPORTUNIDAD SI EXISTE INGRESA
			{
				...
				$query3 = "SELECT id_c, id_usuario_c, usuario_c 
									 FROM m1_oportunidad_productos_cstm 
									 where id_oportunidad_c = '".$id_op."' and id_producto_c = '".$id_producto."'";				
				...
				{//UPDATE 						
					...
					{//CUANDO SE MODIFICA EL DETALLE DE UNA OPORTUNIDAD CERRADA AL 100% 
						if($guarda_o_no == 1) 
						{ 
							$up_query_1 = "UPDATE m1_oportunidad_productos
											SET
				...
				{//INSERT SI NO EXISTE EL DETALLE DEL PRODUCTO PARA LA OPORTUNIDAD
...
function valida_asignados($nombre_oportunidad,$id_usuario_actual,$nombre_cuenta,$guarda_o_no,$ids_validar,$op_duplicado,$duenio)
{
	$resultado = "";
	$id_oportunidad_old = '';
	$probabilidad	='0';
					...
					else 
					{
						if($id_oportunidad_old != '')
						{
							$SQL_X  = "SELECT id_usuario_c,usuario_c 
										FROM m1_oportunidad_productos_cstm 
										WHERE id_producto_c = '".$id_producto."' AND id_oportunidad_c = '".$id_oportunidad_old."' and estado_c = '1'";
			...
			{	
				...
				if($row5=$db->fetchByAssoc($result5))
				{
					...
					else
					{
						...
						if($id_oportunidad_old != '')
						{
							//echo $guarda_o_no." - ".$nombres_usuario2." - ".$id_oportunidad_old." - ".$probabilidad;
							$SQL_X  = "SELECT id_usuario_c,usuario_c 
										FROM m1_oportunidad_productos_cstm 
										WHERE id_producto_c = '".$id_producto."' AND id_oportunidad_c = '".$id_oportunidad_old."'";								
							$result_SQL_X = $db->query($SQL_X, true, 'Error selecting the OLD opportunity record');
...
if($seccion == 'V')
{
	...
	$resp2 = envia_mail();
	$resp3 = guardafeed2($id_oportunidad,$oportunidad,$cuenta_bean->id,$cuenta_bean->name);
	echo $row_val_0['val'].'/'.$id_op;	
}
...
function guardafeed2($id_oportunidad,$oportunidad,$cuenta_id,$cuenta_name)
{
	$db =  DBManagerFactory::getInstance();
	$duenio = $_POST['duenio'];
	$id_creador = "";
