...
while($row=$db->fetchByAssoc($result))
{
	...
	{
		$query2 = "SELECT a.first_name, a.last_name, b.producttemplate_id_c FROM users a, detalle_asig_producto_cstm b, detalle_asig_producto c WHERE b.account_id_c = '".$cuenta_bean->id."' AND b.producttemplate_id_c = '".$row['id_producto']."' AND b.estatus_c = 'Aprobado' AND b.id_c = c.id AND a.id = c.assigned_user_id";
	...
	{
		$query2 = "SELECT usuario_c, id_producto_c FROM m1_oportunidad_productos_cstm WHERE oportunidad_c = '".$nombre_oportunidad."' and id_cuenta_c = '".$cuenta_bean->id."' AND id_producto_c = '".$row['id_producto']."' AND estado_c = '1'";
		$result2 = $db->query($query2, true, 'Error selecting the user owner record');
		if($row2=$db->fetchByAssoc($result2))
		{
