...
$ordenar_por_2 = $_POST['ordenar_por_2'];
$ordenar_por_3 = $_POST['ordenar_por_3'];
$semanal_mensual = $_POST['semanal_mensual'];
$ordenar="";
switch($ordenar_por)
{
	case "Probabilidad":
		$ordenar = 'ORDER BY a.mes_c+0 ASC, c.probability DESC';
		break;
	case "Producto":
		$ordenar = 'ORDER BY a.mes_c+0 ASC, a.producto_c ASC';
		break;
	case "Prospeccion":
		$ordenar = 'ORDER BY a.mes_c+0 ASC, a.valor_prospectado_c DESC';
		break;
	case "Relacionador":
		$ordenar = 'ORDER BY a.mes_c+0 ASC, a.usuario_c ASC';
		break;
}
switch($ordenar_por_2)
{
	case "Probabilidad":
		$ordenar .= ', c.probability DESC';
		break;
	case "Producto":
		$ordenar .= ', a.producto_c ASC';
		break;
	case "Prospeccion":
		$ordenar .= ', a.valor_prospectado_c DESC';
		break;
	case "Relacionador":
		$ordenar .= ', a.usuario_c ASC';
		break;
}
switch($ordenar_por_3)
{
	case "Probabilidad":
		$ordenar .= ', c.probability DESC';
		break;
...
AND a.estatus_c in ('Aprobado','Solicitar')
AND a.tipo_c = 'distribucion'
AND a.valor_c != 0.000000";
...
{
	$resultado.="SELECT a.anio_c as Anio, a.mes_c as Mes, a.usuario_c as Relacionador, CONCAT(d.first_name,' ',d.last_name) as Farming, e.name as Cuenta, a.producto_c as Producto, c.name as Oportunidad, a.valor_prospectado_c as Prospeccion, c.probability as Probabilidad, c.id AS id_Op
...
AND c.id = f.opportunity_id";
	...
	if($cuenta != '')
	{
		$resultado.= "AND a.id_cuenta_c = '".$cuenta."'\n";
	}
	if($result_relacionadores != 'AND a.id_usuario_c IN)')
	{
		$resultado.=$result_relacionadores."\n";
...
{
$resultado.="SELECT d.name AS Cuenta, a.nombre_producto_c AS producto, CONCAT(c.first_name,' ',c.last_name) AS Usuario
FROM detalle_asig_producto_cstm a, detalle_asig_producto b, users c, accounts d, users_cstm e
WHERE b.assigned_user_id = c.id
AND b.id = a.id_c
AND d.id = a.account_id_c
AND d.deleted = '0'
AND c.deleted = '0'
AND c.status = 'Active'
AND a.estatus_c = 'Aprobado'
...
AND c.deleted = '0'";
$resultado.= "\nAND a.nombre_periodo_c = '".$periodo."'\n";
$resultado.= "ORDER BY a.id_mes_c+0\n";
...
ORDER BY a.id_mes_c+0, CONCAT(d.first_name,' ',d.last_name) asc\n";
}
...
{
		$resultado.="SELECT b.name as Empresa, SUM(e.valor_prospectado_c) AS Ventas, e.usuario_c as Relacionador, CONCAT(c.calle_principal_c,' ',c.numero_c,' ',c.calle_secundaria_c) AS Direccion, b.phone_office as Telefono, c.ciudad_c as Ciudad
