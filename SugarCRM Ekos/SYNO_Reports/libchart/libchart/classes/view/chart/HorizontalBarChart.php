<?php
    ...
    class HorizontalBarChart extends BarChart {
         ...
         */
        public function HorizontalBarChart($width, $height) {
            parent::BarChart($width, $height);

            $this->emptyToFullRatio = 1 / 5;
            $this->plot->setGraphPadding(new Padding(5, 50, 30, 150));
        }
            ...
            for ($j = 0; $j < $serieCount; $j++) {
                ...
                for ($i = 0; $i < $pointCount; $i++) {
                    ...
                    if ($this->config->getShowPointCaption()) {
                        ...
                        $text->printText($img, $xmax + 5, $y2 - $barWidth / 2, $this->plot->getTextColor(), $total, $text->fontCondensed, $text->VERTICAL_CENTER_ALIGN);
                    }
