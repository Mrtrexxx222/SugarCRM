...
class Login {
    ...
    public $campo_clave='contrasenia'; //campo que contiene la contraseña
    public $metodo_encriptacion='md5'; //método utilizado para almacenar la contrasela. Opciones: sha1, md5, o texto
   ...
   public function __destruct() {
    ...
    public function login($usuario, $password) {
        ...
        $query='SELECT '.$this->campo_usuario.', '.$this->campo_clave.',id_usuario,tipo_clave,fecha_clave_temp,correo,empresa FROM '.$this->tabla.' WHERE estado=1 and '.$this->campo_usuario.'="'.  mysql_real_escape_string($usuario).'" LIMIT 1 '; //la tabla y el campo se definen en los parametros globales
        ...
        if ($row) {
            ...
            if ($hash==$row[$this->campo_clave]) {
                @session_start();
				$_SESSION['USUARIO']= array('user'=>$row[$this->campo_usuario],
											'id'=>$row['id_usuario'],
											'fecha_c'=>$row['fecha_clave_temp'],
											't_clave'=>$row['tipo_clave'],
											'pass'=>$row[$this->campo_clave],
											'mail'=>$row['correo']); //almacenamos en memoria los datos del Usuario
